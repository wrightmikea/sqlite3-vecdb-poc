...
async fn build_info_handler() -> Response {
    ...
    let build_info = std::fs::read_to_string("static/build-info.js").unwrap_or_else(|_| {
        "window.BUILD_INFO = { host: 'unknown', commit: 'unknown', timestamp: 'unknown' };"
            .to_string()
    });
...
async fn favicon_handler() -> Response {
    match std::fs::read("static/favicon.ico") {
        Ok(data) => (StatusCode::OK, [("Content-Type", "image/x-icon")], data).into_response(),
